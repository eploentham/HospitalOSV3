 SELECT r_uc_plan_group.uc_plan_group_description   
    ,CASE WHEN (sum(query2.thalassemia_infant) > 0)  
        THEN sum(query2.thalassemia_infant)   ELSE 0  END AS thalassemia_infant  
    ,CASE WHEN (sum(query2.thiroid) > 0)   
        THEN sum(query2.thiroid)     ELSE 0  END AS thiroid   
 FROM   r_uc_plan_group 
    LEFT JOIN (SELECT  
 		...
 		WHEN ((query1.plan_group_number = '3' ) OR (query1.plan_group_number = '4' ) ) 
                    AND ((query1.patient_payment_main_hospital  IN (Select  hospital_incup_code  from r_hospital_incup))
                        OR (query1.patient_payment_sub_hospital IN  (Select  hospital_incup_code from r_hospital_incup)))  
                    THEN '3'  
 		WHEN (((query1.plan_group_number = '3' ) OR (query1.plan_group_number = '4' ) ) 
                    ...
                    THEN '4'  
 		WHEN ((query1.plan_group_number <> '1' ) AND (query1.plan_group_number <> '2' )  
                    AND (query1.plan_group_number <> '3' ) AND (query1.plan_group_number <> '4' ) ) 
                    THEN  '5'  	END AS plan_group_id 
            ,CASE WHEN (( query1.r_rp11_disease_id = '8280000000003')   				
                    AND query1.t_patient_id IN (SELECT  t_patient.t_patient_id FROM t_health_family  						
 			...
 			AND t_visit.f_visit_status_id <> '4' 
                    ...
                    GROUP BY t_patient.t_patient_id )) THEN 1 
                    ELSE 0 END AS thalassemia_infant  
 			...
 			WHERE r_rp11_disease.r_rp11_disease_id = '8280000000005' 
                            AND (to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
                                                        ,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)year') ),'999') IS NULL 					
                            AND ((to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
                                                        ,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)mon') ),'999') <=1) 
                            OR (to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
                                                        ,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)mon') ),'999') IS NULL) 
                                                )) 
                            ...
                            AND t_order.f_order_status_id <> '3' 
                            AND t_diag_icd10.diag_icd10_active = '1' 
                            AND SUBSTRING(t_visit.visit_financial_discharge_time,1,10)  < ? GROUP BY  t_patient.t_patient_id ) 
                            AND (to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
                                ,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)year') ),'999') IS NULL 					
                            AND ((to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
                                ,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)mon') ),'999') <=1) 
		...
		((SELECT  t_visit.t_visit_id 
 		 ...
 		 UNION 
                    ...
                    WHERE  r_rp11_disease.r_rp11_disease_id = '8280000000005' 
 			AND (to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
 				,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD'))
                                from '(...)year') ),'999') IS NULL
                        AND ((to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
 				,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)mon') ),'999') <=1) 
                            OR (to_number((substring(' ' || age(to_date(substring(t_visit.visit_financial_discharge_time,1,10)  
 				,'YYYY-MM-DD'), to_date(t_health_family.patient_birthday,'YYYY-MM-DD')) from '(...)mon') ),'999') IS NULL) 
 			...
 			AND t_diag_icd10.diag_icd10_active = '1' 
 			AND SUBSTRING(t_visit.visit_financial_discharge_time,1,10)  < ?
                    GROUP BY t_visit.t_visit_id )) AS query_t	ON query_t.t_visit_id = t_visit.t_visit_id
