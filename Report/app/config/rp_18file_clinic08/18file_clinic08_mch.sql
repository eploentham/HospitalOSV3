...
from (SELECT  
	...
	, CASE 
                    WHEN (length(t_health_pregnancy.health_pregnancy_menses_issue_date)>=10)
                        THEN   (to_number(substring(t_health_pregnancy.health_pregnancy_menses_issue_date,1,4),'9999')-543)               
	...
	, CASE
                    WHEN (length(t_health_pregnancy.health_pregnancy_menses_expire_date)>=10)
                        THEN   (to_number(substring(t_health_pregnancy.health_pregnancy_menses_expire_date,1,4),'9999')-543)                
	...
	,  CASE WHEN (length(t_health_anc.health_anc_hct_date)>=10)
            THEN   (to_number(substring(t_health_anc.health_anc_hct_date,1,4),'9999')-543)               
                            ||   substring(t_health_anc.health_anc_hct_date,6,2)                
                            ||   substring(t_health_anc.health_anc_hct_date,9,2)  
            ELSE ''   END AS DATEHCT  
	...
	,CASE  WHEN (length(SUB0.update_date_time)>=10)
			THEN (to_number(substring(SUB0.update_date_time,1,4),'9999')-543) 
				|| substring(SUB0.update_date_time,6,2)  
				|| substring(SUB0.update_date_time,9,2)         
		ELSE ''   END AS  UPDATE 	

,CASE WHEN length(SUB0.record_date_time)>=10
...
,SUB0.health_postpartum_active as active
FROM  t_health_postpartum  AS SUB0 
    ...
    LEFT JOIN t_health_delivery  ON (cast(t_health_delivery.gravida_number as numeric) = cast(SUB0.health_postpartum_pregnant_number as numeric)
        AND t_health_delivery.t_health_family_id = SUB0.t_health_family_id
        AND t_health_delivery.health_delivery_active = '1')
	LEFT JOIN t_health_pregnancy    ON (cast(SUB0.health_postpartum_pregnant_number as numeric) = cast(t_health_pregnancy.health_pregnancy_gravida_number as numeric)
...
WHERE  
SUB0.health_postpartum_active = '1'    AND 
