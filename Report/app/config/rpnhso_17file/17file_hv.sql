...
b_site.b_visit_office_id 
        		|| substring(t_nhso_service_pp_out.service_date,6,2)       
                || substring((to_number(substring(t_nhso_service_pp_out.service_date,1,4),'9999')-543),3,2)
|| substring(t_nhso_home_visit.t_nhso_home_visit_id,11) as hv_id
,b_site.b_visit_office_id 
        		|| substring(t_nhso_service_pp_out.service_date,6,2)       
                || substring((to_number(substring(t_nhso_service_pp_out.service_date,1,4),'9999')-543),3,2)
||  substr(t_nhso_service_pp_out.t_nhso_service_pp_out_id,11) as t_service_id
