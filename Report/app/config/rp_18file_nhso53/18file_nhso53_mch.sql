...
SELECT  
	...
	, SUB0.health_postpartum_pregnant_number AS GRAVIDA --NOT NULL	
	...
	,  min(CASE WHEN (length(t_health_anc.health_anc_hct_date)>=10)
            then to_char(to_date(to_number(
	...
	, min( CASE WHEN t_health_anc.health_anc_hct_result  is not null
            THEN t_health_anc.health_anc_hct_result    ELSE '0' END)  AS HCT_RS
...
WHERE  health_family_active = '1'
    ...
    AND substring(SUB0.record_date_time,1,10) <= ?
--    AND substring( SUB0.record_date_time,1,10) >= '2551-09-01'        
--    AND substring( SUB0.record_date_time,1,10) <= '2553-09-31'
