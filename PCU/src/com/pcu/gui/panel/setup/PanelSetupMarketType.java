/*
 * PanelSetupMarketType.java
 *
 * Created on 8 กรกฎาคม 2548, 17:57 น.
 */
 ...
 * @author  amp
 */
public class PanelSetupMarketType extends javax.swing.JPanel// implements setEnableForLiftResp
{
    private SetupPcuControl theSetupPcuControl;
    private MarketType theMarketType;
    private Vector vMarketType = new Vector();
    private boolean Forlift = false;
    private HosManage theHM;
    private UpdateStatus theUS;
    
    /** Creates new form PanelSetupMarketType */
    public PanelSetupMarketType(HosManage hm) 
    {
        theHM = hm;
    ...
    private void initComponents() {
        ...
        jTableMarketType = new javax.swing.JTable();
        ...
        jLabel1.setText("MarketTypeSetup");
        ...
        jTableMarketType.setFont(defaultFont1);
        jTableMarketType.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableMarketType.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTableMarketTypeMouseReleased(evt);
            }
        });
        jTableMarketType.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableMarketTypeKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableMarketType);
    ...
    private void jTableMarketTypeKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableMarketTypeKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableMarketTypeKeyReleased

    private void jTableMarketTypeMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableMarketTypeMouseReleased
        select();
    }//GEN-LAST:event_jTableMarketTypeMouseReleased
    ...
    private javax.swing.JTable jTableMarketType;
    ...
    private void save()
    {
        this.theMarketType.number = this.jTextFieldNumber.getText();
        this.theMarketType.description = this.jTextFieldName.getText();        
        if("".equalsIgnoreCase(theMarketType.number))
        {
            theUS.setStatus("กรุณาระบุลำดับ", 2);
            return;
        }
        if("".equalsIgnoreCase(theMarketType.description))
        {
            theUS.setStatus("กรุณากรอกรายละเอียด", 2);
            return;
        }
        if(this.jCheckBoxActive.isSelected())
            this.theMarketType.active = "1";
        else
            this.theMarketType.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listMarketTypeByNameOrNumberAndActive("", this.theMarketType.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            MarketType mt = (MarketType)vList.elementAt(i);
            if(this.theMarketType.getObjectId()==null
                    && this.theMarketType.description.equalsIgnoreCase(mt.description))
            {
                this.theUS.setStatus("ชื่อประเภทของตลาดนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theMarketType.description.equalsIgnoreCase(mt.description)
            &&!this.theMarketType.getObjectId().equalsIgnoreCase(mt.getObjectId()))
            {
                this.theUS.setStatus("ชื่อประเภทของตลาดนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theMarketType.getObjectId()==null
                    && this.theMarketType.number.equalsIgnoreCase(mt.number))
            {
                this.theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.theMarketType.number.equalsIgnoreCase(mt.number)
            &&!this.theMarketType.getObjectId().equalsIgnoreCase(mt.getObjectId()))
            {
        ...
        this.theSetupPcuControl.saveMarketType(this.theMarketType);
        this.theMarketType = null;
    ...
    private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vMarketType =  this.theSetupPcuControl.listMarketTypeByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("description")};
                    
        MarketType marketTypeTemp = new MarketType();
        TaBleModel tm;
        
        if(this.vMarketType != null)
        {   
            tm = new TaBleModel(col,this.vMarketType.size());
            for(int i=0, size=this.vMarketType.size(); i<size; i++)
            {  
                marketTypeTemp = (MarketType)this.vMarketType.get(i);
                tm.setValueAt(marketTypeTemp.number,i,0); 
                tm.setValueAt(marketTypeTemp.description,i,1);                
            }
        }
        else
        {   
        ...
        marketTypeTemp = null;
        this.jTableMarketType.setModel(tm);  
        jTableMarketType.setRowHeight(20);
        /***SetTableDefault***/
        jTableMarketType.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTableMarketType.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
    
    private void select()
    {
        int row = this.jTableMarketType.getSelectedRow();
        this.theMarketType = (MarketType)this.vMarketType.get(row); 
        
        this.jTextFieldName.setText(this.theMarketType.description);
        this.jTextFieldNumber.setText(this.theMarketType.number);
        if(this.theMarketType.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deleteMarketType(this.theMarketType);
        this.theMarketType = null;
    ...
    private void add()
    {
        clearGui();
        setEnableGui(true);
        this.theMarketType = new MarketType();
    }
