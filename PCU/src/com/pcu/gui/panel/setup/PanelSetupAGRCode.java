/*
 * PanelSetupAGRCode.java
 *
 * Created on 25 กรกฎาคม 2548, 11:21 น.
 */
 ...
 * @author  amp
 */
public class PanelSetupAGRCode extends javax.swing.JPanel implements KeyListener// implements setEnableForLiftResp
{
    private SetupPcuControl theSetupPcuControl;
    private AGRCode theAGRCode;
    private Vector vAGRCode = new Vector();
    private boolean Forlift = false;
    private HosManage theHM;
    private UpdateStatus theUS;
    /** Creates new form PanelSetupAGRCode */
    public PanelSetupAGRCode(HosManage hm)
    {
        theHM = hm;
    ...
    private void initComponents() {
        ...
        jTableAGRCode = new javax.swing.JTable();
        ...
        getAccessibleContext().setAccessibleName("\u0e2b\u0e01\u0e14");
        getAccessibleContext().setAccessibleDescription("\u0e01\u0e14\u0e40\u0e49\u0e48");
        jPanel1.setLayout(new java.awt.GridBagLayout());

        jLabel1.setFont(fontTitle1);
        jLabel1.setText("AGRCodeSetup");
        ...
        jTableAGRCode.setFont(defaultFont1);
        jTableAGRCode.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableAGRCode.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTableAGRCodeMouseReleased(evt);
            }
        });
        jTableAGRCode.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableAGRCodeKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableAGRCode);
    ...
    private void jTableAGRCodeKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableAGRCodeKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableAGRCodeKeyReleased
    ...
    private void jTableAGRCodeMouseReleased(java.awt.event.MouseEvent evt)//GEN-FIRST:event_jTableAGRCodeMouseReleased
    {//GEN-HEADEREND:event_jTableAGRCodeMouseReleased
        select();
    }//GEN-LAST:event_jTableAGRCodeMouseReleased
    ...
    private javax.swing.JTable jTableAGRCode;
    ...
    private void save()
    {
        this.theAGRCode.number = this.jTextFieldNumber.getText();
        this.theAGRCode.description = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(theAGRCode.number))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
            return;
        }
        if("".equalsIgnoreCase(theAGRCode.description))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณากรอกรายละเอียด"), 2);
            return;
        }
        if(this.jCheckBoxActive.isSelected())
            this.theAGRCode.active = "1";
        else
            this.theAGRCode.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listAGRCodeByNameOrNumberAndActive("", this.theAGRCode.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            AGRCode agr = (AGRCode)vList.elementAt(i);
            if(this.theAGRCode.getObjectId()==null
                    && this.theAGRCode.description.equalsIgnoreCase(agr.description))
            {
                theUS.setStatus("ชื่อรหัสกลุ่มกิจกรรมนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theAGRCode.description.equalsIgnoreCase(agr.description)
            &&!this.theAGRCode.getObjectId().equalsIgnoreCase(agr.getObjectId()))
            {
                theUS.setStatus("ชื่อรหัสกลุ่มกิจกรรมนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theAGRCode.getObjectId()==null
                    && this.theAGRCode.number.equalsIgnoreCase(agr.number))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.theAGRCode.number.equalsIgnoreCase(agr.number)
            &&!this.theAGRCode.getObjectId().equalsIgnoreCase(agr.getObjectId()))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
        ...
        this.theSetupPcuControl.saveAGRCode(this.theAGRCode);
        this.theAGRCode = null;
    ...
    private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vAGRCode =  this.theSetupPcuControl.listAGRCodeByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("Description")};
                    
        AGRCode aGRCodeTemp = new AGRCode();
        TaBleModel tm;
        
        if(this.vAGRCode != null)
        {   
            tm = new TaBleModel(col,this.vAGRCode.size());
            for(int i=0, size=this.vAGRCode.size(); i<size; i++)
            {  
                aGRCodeTemp = (AGRCode)this.vAGRCode.get(i);
                tm.setValueAt(aGRCodeTemp.number,i,0); 
                tm.setValueAt(aGRCodeTemp.description,i,1);                
            }
        }
        else
        {   
        ...
        aGRCodeTemp = null;
        this.jTableAGRCode.setModel(tm);  
        jTableAGRCode.setRowHeight(20);
        /***SetTableDefault***/
        jTableAGRCode.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTableAGRCode.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
    
    private void select()
    {
        int row = this.jTableAGRCode.getSelectedRow();
        this.theAGRCode = (AGRCode)this.vAGRCode.get(row); 
        
        this.jTextFieldName.setText(this.theAGRCode.description);
        this.jTextFieldNumber.setText(this.theAGRCode.number);
        if(this.theAGRCode.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deleteAGRCode(this.theAGRCode);
        this.theAGRCode = null;
    ...
    private void setKeyBoard()
    {
        ...
        jTableAGRCode.addKeyListener(this);
