/*
 * PanelSetupTempleType.java
 *
 * Created on 4 กรกฎาคม 2548, 11:28 น.
 */
 ...
 * @author  amp
 */
public class PanelSetupTempleType extends javax.swing.JPanel //implements setEnableForLiftResp
{
    private SetupPcuControl theSetupPcuControl;
    private TempleType theTempleType;
    private Vector vTempleType = new Vector();
    private boolean Forlift = false;
    private HosManage theHM;
    private UpdateStatus theUS;
    
    /** Creates new form PanelSetupTempleType */
    public PanelSetupTempleType(HosManage hm) 
    {
        theHM = hm;
...
/**ใช้ในการกำหนดให้ปุ่มลบสามารถทำงานได้หรือไม่
    ...
    private void initComponents() {
        ...
        jTableTempleType = new javax.swing.JTable();
        ...
        jLabel1.setText("TempleTypeSetup");
        ...
        jTableTempleType.setFont(defaultFont1);
        jTableTempleType.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableTempleType.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTableTempleTypeMouseReleased(evt);
            }
        });
        jTableTempleType.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableTempleTypeKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableTempleType);
    ...
    private void jTableTempleTypeKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableTempleTypeKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableTempleTypeKeyReleased

    private void jTableTempleTypeMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableTempleTypeMouseReleased
        select();
    }//GEN-LAST:event_jTableTempleTypeMouseReleased
    ...
    private javax.swing.JTable jTableTempleType;
    ...
    private void save()
    {
        this.theTempleType.number = this.jTextFieldNumber.getText();
        this.theTempleType.description = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(theTempleType.number))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
            return;
        }
        if("".equalsIgnoreCase(theTempleType.description))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณากรอกรายละเอียด"), 2);
            return;
        }
        if(this.jCheckBoxActive.isSelected())
            this.theTempleType.active = "1";
        else
            this.theTempleType.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listTempleTypeByNameOrNumberAndActive("", this.theTempleType.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            TempleType tt = (TempleType)vList.elementAt(i);
            if(this.theTempleType.getObjectId()==null
                    && this.theTempleType.description.equalsIgnoreCase(tt.description))
            {
                theUS.setStatus("ชื่อประเภทของศาสนสถานนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theTempleType.description.equalsIgnoreCase(tt.description)
            &&!this.theTempleType.getObjectId().equalsIgnoreCase(tt.getObjectId()))
            {
                theUS.setStatus("ชื่อประเภทของศาสนสถานนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theTempleType.getObjectId()==null
                    && this.theTempleType.number.equalsIgnoreCase(tt.number))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.theTempleType.number.equalsIgnoreCase(tt.number)
            &&!this.theTempleType.getObjectId().equalsIgnoreCase(tt.getObjectId()))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
        ...
        this.theSetupPcuControl.saveTempleType(this.theTempleType);
        this.theTempleType = null;
    ...
    private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vTempleType =  this.theSetupPcuControl.listTempleTypeByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("Description")};
                    
        TempleType templeTypeTemp = new TempleType();
        TaBleModel tm;
        
        if(this.vTempleType != null)
        {   
            tm = new TaBleModel(col,this.vTempleType.size());
            for(int i=0, size=this.vTempleType.size(); i<size; i++)
            {  
                templeTypeTemp = (TempleType)this.vTempleType.get(i);
                tm.setValueAt(templeTypeTemp.number,i,0); 
                tm.setValueAt(templeTypeTemp.description,i,1);                
            }
        }
        else
        {   
        ...
        templeTypeTemp = null;
        this.jTableTempleType.setModel(tm);  
        jTableTempleType.setRowHeight(20);
        /***SetTableDefault***/
        jTableTempleType.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTableTempleType.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
    
    private void select()
    {
        int row = this.jTableTempleType.getSelectedRow();
        this.theTempleType = (TempleType)this.vTempleType.get(row); 
        
        this.jTextFieldName.setText(this.theTempleType.description);
        this.jTextFieldNumber.setText(this.theTempleType.number);
        if(this.theTempleType.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deleteTempleType(this.theTempleType);
        this.theTempleType = null;
    ...
    private void add()
    {
        clearGui();
        setEnableGui(true);
        this.theTempleType = new TempleType();
    }
