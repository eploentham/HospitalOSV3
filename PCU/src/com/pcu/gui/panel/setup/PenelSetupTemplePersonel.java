/*
 * PenelSetupTemplePersonel.java
 *
 * Created on 4 กรกฎาคม 2548, 12:59 น.
 */
 ...
 * @author  amp
 */
public class PenelSetupTemplePersonel extends javax.swing.JPanel// implements setEnableForLiftResp
{
    private SetupPcuControl theSetupPcuControl;
    private TemplePersonel theTemplePersonel;
    private Vector vTemplePersonel = new Vector();
    private boolean Forlift = false;
    private HosManage theHM;
    /** Creates new form PenelSetupTemplePersonel */
    public PenelSetupTemplePersonel(HosManage hm) 
    {
        theHM = hm;
    ...
    private void initComponents() {
        ...
        jTableTemplePersonel = new javax.swing.JTable();
        ...
        jLabel1.setText("TemplePersonelSetup");
        ...
        jTableTemplePersonel.setFont(defaultFont1);
        jTableTemplePersonel.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableTemplePersonel.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTableTemplePersonelMouseReleased(evt);
            }
        });
        jTableTemplePersonel.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableTemplePersonelKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableTemplePersonel);
    ...
    private void jTableTemplePersonelKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableTemplePersonelKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableTemplePersonelKeyReleased

    private void jTableTemplePersonelMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableTemplePersonelMouseReleased
        select();
    }//GEN-LAST:event_jTableTemplePersonelMouseReleased
    ...
    private javax.swing.JTable jTableTemplePersonel;
    ...
    private void save()
    {
        this.theTemplePersonel.number = this.jTextFieldNumber.getText();
        this.theTemplePersonel.description = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(theTemplePersonel.number))
        {
           theHM.theHosInf.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
           jTextFieldNumber.requestFocus();
           return;
        }       
        if("".equalsIgnoreCase(theTemplePersonel.description))
        {
        ...
        if(this.jCheckBoxActive.isSelected())
            this.theTemplePersonel.active = "1";
        else
            this.theTemplePersonel.active = "0";
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listTemplePersonelByNameOrNumberAndActive("", this.theTemplePersonel.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            TemplePersonel tp = (TemplePersonel)vList.elementAt(i);
            if(this.theTemplePersonel.getObjectId()==null
                    && this.theTemplePersonel.description.equalsIgnoreCase(tp.description))
            {
                this.theHM.theHosInf.setStatus("ชื่อบุคคลประจำศาสนสถานนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theTemplePersonel.description.equalsIgnoreCase(tp.description)
            &&!this.theTemplePersonel.getObjectId().equalsIgnoreCase(tp.getObjectId()))
            {
                this.theHM.theHosInf.setStatus("ชื่อบุคคลประจำศาสนสถานนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theTemplePersonel.getObjectId()==null
                    && this.theTemplePersonel.number.equalsIgnoreCase(tp.number))
            {
                this.theHM.theHosInf.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.theTemplePersonel.number.equalsIgnoreCase(tp.number)
            &&!this.theTemplePersonel.getObjectId().equalsIgnoreCase(tp.getObjectId()))
            {
        ...
        this.theSetupPcuControl.saveTemplePersonel(this.theTemplePersonel);
        this.theTemplePersonel = null;
    ...
    private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vTemplePersonel =  this.theSetupPcuControl.listTemplePersonelByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("Description")};
                    
        TemplePersonel templePersonelTemp = new TemplePersonel();
        TaBleModel tm;
        
        if(this.vTemplePersonel != null)
        {   
            tm = new TaBleModel(col,this.vTemplePersonel.size());
            for(int i=0, size=this.vTemplePersonel.size(); i<size; i++)
            {  
                templePersonelTemp = (TemplePersonel)this.vTemplePersonel.get(i);
                tm.setValueAt(templePersonelTemp.number,i,0); 
                tm.setValueAt(templePersonelTemp.description,i,1);                
            }
        }
        else
        {   
        ...
        templePersonelTemp = null;
        this.jTableTemplePersonel.setModel(tm);  
        jTableTemplePersonel.setRowHeight(20);
        /***SetTableDefault***/
        jTableTemplePersonel.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTableTemplePersonel.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
    
    private void select()
    {
        int row = this.jTableTemplePersonel.getSelectedRow();
        this.theTemplePersonel = (TemplePersonel)this.vTemplePersonel.get(row); 
        
        this.jTextFieldName.setText(this.theTemplePersonel.description);
        this.jTextFieldNumber.setText(this.theTemplePersonel.number);
        if(this.theTemplePersonel.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deleteTemplePersonel(this.theTemplePersonel);
        this.theTemplePersonel = null;
    ...
    private void add()
    {
        clearGui();
        setEnableGui(true);
        this.theTemplePersonel = new TemplePersonel();
    }
