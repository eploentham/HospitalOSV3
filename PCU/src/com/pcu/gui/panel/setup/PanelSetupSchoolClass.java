/*
 * PanelSetupSchoolClass.java
 *
 * Created on 9 สิงหาคม 2548, 9:36 น.
 */
 ...
 * @author  Noom
 */
public class PanelSetupSchoolClass extends javax.swing.JPanel {
    private SetupPcuControl theSetupPcuControl;
    private SchoolClass theSchoolClass;
    private Vector vSchoolClass = new Vector();
    private boolean flagButtonDelete = false;
    UpdateStatus theUS;
    
    /** Creates new form PanelSetupSchoolClass */
    public PanelSetupSchoolClass() {
        initComponents();
    }
    
     /** Creates new form PanelSetupSchoolClass */
    public PanelSetupSchoolClass(PCUControl hc) 
    {
        theUS = hc.theUS;
     ...
     private void setEnableGui(boolean flag)
    {
        ...
        this.jCheckBoxSchoolMaxClass.setEnabled(flag);
          if(flagButtonDelete){
            this.jButtonDelete.setEnabled(!flag);
        }
          else
   ...
   /**ใช้ในการกำหนดให้ปุ่มลบสามารถทำงานได้หรือไม่
    ...
    private void initComponents() {
        ...
        jTableSchoolClass = new javax.swing.JTable();
        ...
        jCheckBoxSchoolMaxClass = new javax.swing.JCheckBox();
        ...
        jLabel1.setText("SchoolClassSetup");
        ...
        jTableSchoolClass.setFont(defaultFont1);
        jTableSchoolClass.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableSchoolClass.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTableSchoolClassMouseReleased(evt);
            }
        });
        jTableSchoolClass.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableSchoolClassKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableSchoolClass);
        ...
        jCheckBoxSchoolMaxClass.setFont(defaultFont1);
        jCheckBoxSchoolMaxClass.setText("SchoolMaxClass");
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 1;
        gridBagConstraints.gridy = 2;
        gridBagConstraints.anchor = java.awt.GridBagConstraints.NORTHWEST;
        gridBagConstraints.insets = new java.awt.Insets(11, 11, 0, 5);
        jPanel7.add(jCheckBoxSchoolMaxClass, gridBagConstraints);
    ...
    private void jTableSchoolClassKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableSchoolClassKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableSchoolClassKeyReleased
    ...
    private void jTableSchoolClassMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableSchoolClassMouseReleased
         select();
    }//GEN-LAST:event_jTableSchoolClassMouseReleased
      ...
      private void setLanguage()
    {
        ...
        jCheckBoxSchoolMaxClass.setText(GutilPCU.getTextBundle(jCheckBoxSchoolMaxClass.getText()));
    }
    ...
    {         
        ...
        PanelSetupSchoolClass thePanelSetupSchoolClass = new PanelSetupSchoolClass(hc);       
        JFrame frm = new JFrame("PanelSetupSchoolClass");        
      
        frm.getContentPane().add(thePanelSetupSchoolClass);       
        frm.pack();
       ...
       private void save()
    {
        this.theSchoolClass.number = this.jTextFieldNumber.getText();
        this.theSchoolClass.description = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(theSchoolClass.number))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
            return;
        }
        if("".equalsIgnoreCase(theSchoolClass.description))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณากรอกรายละเอียด"), 2);
            return;
        }
        if(this.jCheckBoxSchoolMaxClass.isSelected())
            this.theSchoolClass.school_max_class_status = "1";
        else
            this.theSchoolClass.school_max_class_status = "0";
        
        if(this.jCheckBoxActive.isSelected())
            this.theSchoolClass.active = "1";
        else
            this.theSchoolClass.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listSchoolClassByNameOrNumberAndActive("", this.theSchoolClass.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            SchoolClass sc = (SchoolClass)vList.elementAt(i);
            if(this.theSchoolClass.getObjectId()==null
                    && this.theSchoolClass.description.equalsIgnoreCase(sc.description))
            {
                theUS.setStatus("ชื่อระดับชั้นของสถานศึกษานี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theSchoolClass.description.equalsIgnoreCase(sc.description)
            &&!this.theSchoolClass.getObjectId().equalsIgnoreCase(sc.getObjectId()))
            {
                theUS.setStatus("ชื่อระดับชั้นของสถานศึกษานี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theSchoolClass.getObjectId()==null
                    && this.theSchoolClass.number.equalsIgnoreCase(sc.number))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }            
            else if(this.theSchoolClass.number.equalsIgnoreCase(sc.number)
            &&!this.theSchoolClass.getObjectId().equalsIgnoreCase(sc.getObjectId()))
            {
                theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
        ...
        this.theSetupPcuControl.saveSchoolClass(this.theSchoolClass);
        this.theSchoolClass = null;
       ...
       private void search()
       {
           ...
           if(this.jCheckBoxSearchActive.isSelected())
               active = "1";
           this.vSchoolClass =  this.theSetupPcuControl.listSchoolClassByNameOrNumberAndActive(search,active);
           
           /***SetTable***/
           String[] col = {GutilPCU.getTextBundle("Number"),
                   GutilPCU.getTextBundle("Description")};
                   
                   SchoolClass servicePlaceTemp = new SchoolClass();
                   TaBleModel tm;
                   
                   if(this.vSchoolClass != null)
                   {
                       tm = new TaBleModel(col,this.vSchoolClass.size());
                       for(int i=0, size=this.vSchoolClass.size(); i<size; i++)
                       {
                           servicePlaceTemp = (SchoolClass)this.vSchoolClass.get(i);
                   ...
                   this.jTableSchoolClass.setModel(tm);
                   jTableSchoolClass.setRowHeight(20);
                   /***SetTableDefault***/
                   jTableSchoolClass.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
                   jTableSchoolClass.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/
       }
       
       private void select()
       {
           int row = this.jTableSchoolClass.getSelectedRow();
           this.theSchoolClass = (SchoolClass)this.vSchoolClass.get(row);
           
           this.jTextFieldName.setText(this.theSchoolClass.description);
           this.jTextFieldNumber.setText(this.theSchoolClass.number);
           if(this.theSchoolClass.school_max_class_status.equals("1"))
               this.jCheckBoxSchoolMaxClass.setSelected(true);
           else
               this.jCheckBoxSchoolMaxClass.setSelected(false);
           if(this.theSchoolClass.active.equals("1"))
       ...
       private void delete()
       {
           this.theSetupPcuControl.deleteSchoolClass(this.theSchoolClass);
           this.theSchoolClass = null;
       ...
       private void add()
       {
           clearGui();
           setEnableGui(true);
           this.theSchoolClass = new SchoolClass();
       }
    ...
    private javax.swing.JCheckBox jCheckBoxSchoolMaxClass;
    ...
    private javax.swing.JTable jTableSchoolClass;
