/*
 * PanelSetupResource.java
 *
 * Created on 15 กรกฎาคม 2548, 15:11 น.
 */
 ...
 * @author  amp
 */
public class PanelSetupResource extends javax.swing.JPanel// implements setEnableForLiftResp
{
    private SetupPcuControl theSetupPcuControl;
    private CommunityResource theResource;
    private Vector vResource = new Vector();
    private boolean Forlift = false;
    private HosManage theHM;
    private UpdateStatus theUS;
    /** Creates new form PanelSetupResource */
    public PanelSetupResource(HosManage hm) 
    {
        theHM = hm;
    ...
    private void initComponents() {
        ...
        jTableResource.setFont(defaultFont1);
        ...
        jTableResource.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTableResourceKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTableResource);
    ...
    private void jTableResourceKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableResourceKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTableResourceKeyReleased
    ...
    private void save()
    {
        this.theResource.number = this.jTextFieldNumber.getText();
        this.theResource.description = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(theResource.number))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
            return;
        }
        if("".equalsIgnoreCase(theResource.description))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณากรอกรายละเอียด"), 2);
            return;
        }
        if(this.jCheckBoxActive.isSelected())
            this.theResource.active = "1";
        else
            this.theResource.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listResourceByNameOrNumberAndActive("", this.theResource.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            CommunityResource cr = (CommunityResource)vList.elementAt(i);
            if(this.theResource.getObjectId()==null
                    && this.theResource.description.equalsIgnoreCase(cr.description))
            {
                this.theUS.setStatus("ชื่อทรัพยากรนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.theResource.description.equalsIgnoreCase(cr.description)
            &&!this.theResource.getObjectId().equalsIgnoreCase(cr.getObjectId()))
            {
                this.theUS.setStatus("ชื่อทรัพยากรนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.theResource.getObjectId()==null
                    && this.theResource.number.equalsIgnoreCase(cr.number))
            {
                this.theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.theResource.number.equalsIgnoreCase(cr.number)
            &&!this.theResource.getObjectId().equalsIgnoreCase(cr.getObjectId()))
            {
        ...
        this.theSetupPcuControl.saveResource(this.theResource);
    ...
    private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vResource =  this.theSetupPcuControl.listResourceByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("Description")};
                    
        CommunityResource resourceTemp = new CommunityResource();
        TaBleModel tm;
        
        if(this.vResource != null)
        {   
            tm = new TaBleModel(col,this.vResource.size());
            for(int i=0, size=this.vResource.size(); i<size; i++)
            {  
                resourceTemp = (CommunityResource)this.vResource.get(i);
                tm.setValueAt(resourceTemp.number,i,0); 
                tm.setValueAt(resourceTemp.description,i,1);                
            }
        }
        else
        {   
        ...
        resourceTemp = null;
        this.jTableResource.setModel(tm);  
        jTableResource.setRowHeight(20);
        /***SetTableDefault***/
        jTableResource.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTableResource.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
    
    private void select()
    {
        int row = this.jTableResource.getSelectedRow();
        this.theResource = (CommunityResource)this.vResource.get(row); 
        
        this.jTextFieldName.setText(this.theResource.description);
        this.jTextFieldNumber.setText(this.theResource.number);
        if(this.theResource.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deleteResource(this.theResource);
        this.theResource = null;
    ...
    private void add()
    {
        clearGui();
        setEnableGui(true);
        this.theResource = new CommunityResource();
    }
