/*
 * PanelSetupPetType.java
 *
 * Created on 16 มิถุนายน 2548, 10:31 น.
 */
 ...
 */
public class PanelSetupPetType extends javax.swing.JPanel// implements setEnableForLiftResp
{   
    ...
    /** Creates new form PanelSetupPetType */
    public PanelSetupPetType(HosManage hm) 
    {
        theHM = hm;
  ...
  /**ใช้ในการกำหนดให้ปุ่มลบสามารถทำงานได้หรือไม่
    ...
    private void initComponents() {
        ...
        jTablePetType = new javax.swing.JTable();
        ...
        jTablePetType.setFont(defaultFont1);
        jTablePetType.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTablePetType.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                jTablePetTypeMouseReleased(evt);
            }
        });
        jTablePetType.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTablePetTypeKeyReleased(evt);
            }
        });

        jScrollPane1.setViewportView(jTablePetType);
    ...
    private void jTablePetTypeKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTablePetTypeKeyReleased
        if(evt.getKeyCode() == KeyEvent.VK_UP || evt.getKeyCode() == KeyEvent.VK_DOWN)
            select();
    }//GEN-LAST:event_jTablePetTypeKeyReleased
...
// TODO add your handling code here:
    ...
    private void jTablePetTypeMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTablePetTypeMouseReleased
        select();
    }//GEN-LAST:event_jTablePetTypeMouseReleased
    ...
    private javax.swing.JTable jTablePetType;
    ...
    private void save()
    {
        this.thePetType.number = this.jTextFieldNumber.getText();
        this.thePetType.name = this.jTextFieldName.getText();
        if("".equalsIgnoreCase(thePetType.number))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณาระบุลำดับ"), 2);
            return;
        }
        if("".equalsIgnoreCase(thePetType.name))
        {
            theUS.setStatus(GutilPCU.getTextBundle("กรุณากรอกรายละเอียด"), 2);
            return;
        }
        if(this.jCheckBoxActive.isSelected())
            this.thePetType.active = "1";
        else
            this.thePetType.active = "0";
        
        //ตรวจสอบการเพิ่มข้อมูลซ้ำ ปู 20/12/2549
        Vector vList = new Vector();
        vList=  this.theSetupPcuControl.listPetTypeByNameOrNumberAndActive("", this.thePetType.active);
        for(int i=0,size = vList.size();i <size; i++)
        {
            PetType pt = (PetType)vList.elementAt(i);
            if(this.thePetType.getObjectId()==null
                    && this.thePetType.name.equalsIgnoreCase(pt.name))
            {
                this.theUS.setStatus("ชื่อสัตว์เลี้ยงนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            }
            else if(this.thePetType.name.equalsIgnoreCase(pt.name)
            &&!this.thePetType.getObjectId().equalsIgnoreCase(pt.getObjectId()))
            {
                this.theUS.setStatus("ชื่อสัตว์เลี้ยงนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldName.requestFocus();
                return;
            ...
            if(this.thePetType.getObjectId()==null
                    && this.thePetType.number.equalsIgnoreCase(pt.number))
            {
                this.theUS.setStatus("ลำดับนี้มีอยู่ในฐานข้อมูลแล้ว", 2);
                jTextFieldNumber.requestFocus();
                return;
            }
            else if(this.thePetType.number.equalsIgnoreCase(pt.number)
            &&!this.thePetType.getObjectId().equalsIgnoreCase(pt.getObjectId()))
            {
        ...
        this.theSetupPcuControl.savePetType(this.thePetType);
        this.thePetType = null;
     ...
     private void search()
    {
        ...
        if(this.jCheckBoxSearchActive.isSelected())
            active = "1";
        this.vPetType =  this.theSetupPcuControl.listPetTypeByNameOrNumberAndActive(search,active);
        
        /***SetTable***/
        String[] col = {GutilPCU.getTextBundle("Number"),
                    GutilPCU.getTextBundle("name")};
                    
        PetType PetTypeTemp = new PetType();
        TaBleModel tm;
        
        if(this.vPetType != null)
        {   
            tm = new TaBleModel(col,this.vPetType.size());
            for(int i=0, size=this.vPetType.size(); i<size; i++)
            {  
                PetTypeTemp = (PetType)this.vPetType.get(i);
                tm.setValueAt(PetTypeTemp.number,i,0); 
                tm.setValueAt(PetTypeTemp.name,i,1);                
            }
        }
        else
        {   
        ...
        PetTypeTemp = null;
        this.jTablePetType.setModel(tm);  
        jTablePetType.setRowHeight(20);
        /***SetTableDefault***/
        jTablePetType.getColumnModel().getColumn(0).setPreferredWidth(80);      /*รหัสเขตที่ตั้งหมู่บ้าน*/
        jTablePetType.getColumnModel().getColumn(1).setPreferredWidth(200);     /*ชื่อเขตที่ตั้ง*/        
    }
          
    private void select()
    {
        int row = this.jTablePetType.getSelectedRow();
        this.thePetType = (PetType)this.vPetType.get(row); 
        
        this.jTextFieldName.setText(this.thePetType.name);
        this.jTextFieldNumber.setText(this.thePetType.number);
        if(this.thePetType.active.equals("1"))
            this.jCheckBoxActive.setSelected(true);
        else
    ...
    private void delete()
    {
        this.theSetupPcuControl.deletePetType(this.thePetType);
        this.thePetType = null;
