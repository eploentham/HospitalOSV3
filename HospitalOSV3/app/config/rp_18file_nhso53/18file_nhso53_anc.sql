...
FROM 
 (SELECT 				
    ...
    , case when length(t_health_anc.modify_date_time) >= 10
                ...
                then case when length(cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) = 14
                                  ...
                                  when length(cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) =12
                                  then (cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) || '00'
                                  when length(cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) =10
                                  then (cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) || '0000'
                                  when length(cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) = 8
                                  then (cast(substring(t_health_anc.record_date_time,1,4) as numeric) - 543
                                                || replace(replace(replace(substring(t_health_anc.record_date_time,5),'-',''),',',''),':','')) || '000000'
                                  else ''
                           end            
                else ''
...
FROM t_health_anc 			
	...
	INNER JOIN t_visit ON t_health_anc.t_visit_id = t_visit.t_visit_id AND t_visit.f_visit_status_id <> '4'		
...
FROM t_health_anc 			
	INNER JOIN t_health_family ON t_health_anc.t_health_family_id = t_health_family.t_health_family_id      			
	LEFT JOIN t_visit ON t_health_anc.t_visit_id = t_visit.t_visit_id AND t_visit.f_visit_status_id <> '4'				
        LEFT JOIN t_health_pregnancy ON t_health_anc.t_health_pregnancy_id = t_health_pregnancy.t_health_pregnancy_id
	,b_site
WHERE  t_health_anc.health_anc_active = '1'         
    AND health_family_active = '1'   				
    AND (substring(t_health_anc.health_anc_survey,1,10) >= ?  AND substring(t_health_anc.health_anc_survey,1,10) <= ?)
) anc53
