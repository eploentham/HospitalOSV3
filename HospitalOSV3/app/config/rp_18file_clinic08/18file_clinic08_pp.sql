...
from (
SELECT  
	 b_site.b_visit_office_id || substring(t_health_pp.t_health_pp_id,9) as PP_ID
	...
	, CASE WHEN (t_health_pp.pp_lost_oxygen= '0')
				THEN '1'
	...
	, (CASE WHEN SUB1.pp_care_number = '3'
            THEN (to_number(substring(t_visit.visit_begin_visit_time,1,4),'9999')-543)
					...
					||substring(t_visit.visit_begin_visit_time,9,2)
            ELSE '' END) as		 BCARE3
	, CASE WHEN (SUB1.pp_care_result = '1' OR SUB1.pp_care_result = '2')
	...
	, CASE WHEN length(SUB1.pp_care_modify_date_time)>=10
        ...
        ELSE (to_number(substring(SUB1.pp_care_record_date_time,1,4),'9999')-543)
				...
				|| substring(SUB1.pp_care_record_date_time,9,2) END  as UPDATE_DATE 
