 ...
 */
public class R53CardInsTypeLookup implements LookupControlInf{
    private final ConnectionInf theConnectionInf;
    private Vector vret;
    /** Creates a new instance of AccidentTypeLookup */
    public R53CardInsTypeLookup(ConnectionInf con){
    ...
    public java.util.Vector listData(String str) {
        try {
            ...
            ResultSet rs = theConnectionInf.eQuery(
                    "select * from (select id,inscl_name || ' '||maininscl as des " +
                    " from r_rp1853_instype) instype where id ilike '"+str+"'" +
                    " or des ilike '%"+str+"%' order by des");
            vret = new Vector();
            while (rs.next()) {
    ...
    public java.util.Vector listData2(String str) {
        try {
            theConnectionInf.open();
            ResultSet rs = theConnectionInf.eQuery(
                    "select * from (select id,inscl_name || ' '||maininscl as des " +
                    " from r_rp1853_instype) instype where id ilike '"+str+"'" +
                    " or des ilike '%"+str+"%' order by id");
            vret = new Vector();
            while (rs.next()) {
                ComboFix cf = new ComboFix();
                cf.code = rs.getString(1);
