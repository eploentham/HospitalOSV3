...
import com.report12file.object.Odx;
 ...
 * @Modify Ojika
 */
public class OdxDB implements File12DB {
    ...
    /** Creates a new instance of OdxDB */
    public OdxDB() {
    }
    
    public OdxDB(ConnectionInf db) {
        theConnectionInf=db;
        dbObj = new Odx();
    }
    ...
    public Vector selectByDate(String startDate, String endDate,int exp) throws Exception {
        ...
        if(exp==1)
            sql = IOStream.readInputDefault("config/rp_12file50/12file50_odx.sql");
    ...
    public java.util.Vector eQuery(ResultSet rs) throws Exception 
    {
        ...
        while(rs.next()) {
            Odx theodx = new Odx();
            theodx.hn = rs.getString(1);
                    theodx.datedx = rs.getString(2);
                    theodx.clinic = rs.getString(3);
                    theodx.diag = rs.getString(4);
                    theodx.dxtype = rs.getString(5);
                    theodx.drdx = rs.getString(6);
            
             if(exp==1)
                    theodx.pid = rs.getString(7);
            vc.add(theodx);
            theodx = null;
        }
        rs.close();
     ...
     * @param vObject เป็น Vector ของ Odx
     */
    ...
    public Vector convertData(Vector vData) throws Exception {
        ...
        if(vData != null && vData.size() > 0)
        {
            ...
            {
                theOdx = (Odx)vData.get(i);
                if(theOdx != null)
                {
                    // ส่งเข้าไปเปลี่ยนรูปแบบวันที่เป็น คศ.
                    theOdx.datedx = Util.convertYearString(theOdx.datedx);   
                    
                    vConverData.add(theOdx);
                }
                theOdx = null;
            }
        }
